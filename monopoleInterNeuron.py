...
class MonopoleInterNeuron:
    def __init__(self,inputs=None, outputs=None, externalInput = 0.0, position = [0.0, 0.0, 0.0], debug=False, tau=0.1, surfaceArea=1.0, tspan=range((10000)), inactivating=True):
        ...
        self.v_K = -100          #mV
        self.g_shunt = 2 #0.85      #mS/cm^2 (was varied from 2 to 5.3 in the paper)
        self.v_shunt = -70       #mV
        self.gdapt = 0         #mS/cm^2 (for m-current adaptation)
        self.gAHP = 1          #mS/cm^2
        self.vAHP = self.v_K
        self.alphaAHP = 0.005
        self.betaAHP = 0
...
#        self.dv=[-100 for f in range(len(self.tspan))]
    ...
    def step(self, time, externalCurrent = 0.0):
        ...
        # Update Instants
        self.m_inf = 0.5*(1+tanh((self.v-self.v_1)/self.v_2))
        self.tauh = ((2*232/pi)*(28/(4*(self.v+64)**2 + 28**2))) # Equivalent to the equation in Fernandez, Mehaffey, & Turner, 2005
...
#         self.ahp = self.ahp + ((1/(1+exp((self.betaAHP-self.v)/self.gammaAHP))-self.ahp) / (self.tauAHP)) Old implementation, incorrect
        self.ahp = self.ahp + self.alphaAHP * (1/(1+exp((self.betaAHP-self.v)/self.gammaAHP))-self.ahp) # New implementation, from Prescott et al. 2006
