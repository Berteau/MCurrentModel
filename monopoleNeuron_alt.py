...
class MonopoleNeuron:
    def __init__(self,inputs=None, outputs=None, externalInput = 0.0, position = [0.0, 0.0, 0.0], debug=False, tau=0.1, surfaceArea=1.0, tspan=range((10000)), inactivating=True):
        ...
        self.v_K = -100          #mV
        self.g_shunt = 1.5 #0.85      #mS/cm^2 (was varied from 2 to 5.3 in the paper)
        self.v_shunt = -70       #mV
        self.gdapt = 4.0         #mS/cm^2 (for m-current adaptation)
        self.gAHP = 1          #mS/cm^2
        self.vAHP = self.v_K
        self.betaAHP = 0
        self.gammaAHP = 5
...
#        self.beta_h = 1 / (exp((self.v+30)/10)+1)
        ...
        # I_Na
        I_Na = self.surfaceArea*self.g_Na*self.m_inf*(activeNA)*(self.v-self.v_Na)
...
#        self.z = self.z + ((self.z_inf - self.z) / self.tau_z*)
        ...
        if self.inactivating == True:
            ...
            activeNA = 1# (1-self.h)
        self.IExternal = (self.surfaceArea*self.g_Na*self.m_inf*(activeNA)*(self.v-self.v_Na) - self.surfaceArea*self.g_K*self.w*(self.v-self.v_K) - self.surfaceArea*self.g_shunt*(self.v-self.v_shunt) - self.surfaceArea*self.gdapt*self.z*(self.v-self.v_K) )/(self.C*(1/self.tau))
...
#        self.dv (self.dv_temp)
        ...
        # if self.v > -10:
        #     print self.currentlySpiking, self.v
        # Record Info
